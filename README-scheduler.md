...
- `reporters/CustomReporter.ts` - Generates the enterprise HTML report (same design as `Regression_Test`) used both on-disk and for the scheduled email body.
...
- Timestamped nightly run artifacts folder:
  ...
  - Includes:
    - `General_Test-report_yyyyMMdd_HHmmss.zip` (zipped copy of the enterprise report sent by email, for offline viewing with working assets/screenshots)
    - `results.json` (Playwright raw JSON copy)
    - `html-report\index.html` (Playwright HTML report copy)
- Enterprise HTML report (same design used for `Regression_Test`), used both on-disk and as the scheduled email report:
  - `<project-root>\Reports\General_Test\custom-html-report\index.html`
...
- sends the enterprise `Reports\General_Test\custom-html-report\index.html` report (CSS inlined) as the email body, and attaches a zipped copy of that report plus the Excel result file, via Outlook Desktop COM automation
...
The enterprise custom HTML report (same design/layout used for `Regression_Test`) is generated by `CustomReporter` for scheduled runs, but its output set is always saved under:

- `<project-root>\Reports\General_Test\custom-html-report\index.html`

regardless of which spec folder (Regression_Test, Functional_Test, etc.) the executed tests live in. This is the same report that gets emailed after each scheduled run.
