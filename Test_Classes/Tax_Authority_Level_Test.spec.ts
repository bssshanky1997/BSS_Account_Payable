...
test.describe('Tax Authority Level', () => {
  ...
  test.beforeEach(async ({ page }, testInfo) => {
    ...
    // Step 1: Open AP Home and switch company.
    await runStep('Open AP Home and switch company', async () => {
      await apHomePage.openHomePage();
      await apHomePage.changeCompanyId(companyId);
    });

    // Step 2: Configure Company Application Settings (Tax Type / Tax Level / Show fields).
    await runStep('Configure Company Application Settings', async () => {
  ...
  test('Open AP Invoice and choose Create From Scratch', async () => {
    ...
    await runStep('Click Create New Invoice and Create From Scratch', async () => {
      await apHomePage.openCreateFromScratchForm();
    });
    ...
    await runStep('Enter Subtotal 100 and validate Tax/Total amounts', async () => {
      await apHomePage.enterSubtotalAndValidateTaxAndTotal(100);
    });
    ...
    await runStep('Select Tax Authorities and validate auto-calculated Tax/Total', async () => {
      await apHomePage.selectTaxAuthoritiesAndValidateAmounts(100, [
        'TAX_AUTH_2',
        'TAX_AUTH_2',
        'TAX_AUTH_2',
        'TAX_AUTH_2',
    ...
    await runStep('Step 8: Fill Invoice header and select PO reference', async () => {
      await apHomePage.populateInvoiceHeaderAndSelectPoReference('Inv00012');
    });
